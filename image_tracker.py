...
class ImageTracker:
    ...
    def find_robots(self):
        ...
        for position in self.allies_shirt:
            xi, xf = position[0] - offset, position[0] + offset
            yi, yf = position[1] - offset, position[1] + offset
            robot_roi = self.frame[xi:xf, yi:yf]

            # Find robot 0
            mask = cv2.inRange(self.robot_roi, self.segm_limits[1][0], self.segm_limits[1][1])
            mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, self.kernel)
            self.contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_KCOS)
            if self.contours:
                M = cv2.moments(self.contours[0])
                if M['m00'] == 0:
                    ...
                    id_x = int(M['m10']/M['m00'])
                    id_y = int(M['m01']/M['m00'])
                x = (id_x + position[0])/2
                y = (id_y + position[1])/2

                self.allies_position[0] = [x, y]
                self.robots_found[0] = True
                self.allies_angles[0] = find_angle(id_x, id_y, position[0], position[1])
            else:
                # Find robot 1
                mask = cv2.inRange(self.robot_roi, self.segm_limits[2][0], self.segm_limits[2][1])
                mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, self.kernel)
                self.contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_KCOS)
                if self.contours:
                    M = cv2.moments(self.contours[0])
                    ...
                    x = (id_x + position[0])/2
                    y = (id_y + position[1])/2
                    self.allies_position[1] = [x, y]
                    self.robots_found[1] = True
                    self.allies_angles[1] = find_angle(id_x, id_y, position[0], position[1])
                else:
                    # Find robot 2
                    mask = cv2.inRange(self.robot_roi, self.segm_limits[3][0], self.segm_limits[3][1])
                    mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, self.kernel)
                    self.contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_KCOS)
                    if self.contours:
                        M = cv2.moments(self.contours[0])
    ...
    #def find_angle(self, id_x, id_y, shirt_x, shirt_y):
        ...
        dx = id_x - shirt_x
        dy = -1 * (id_y - shirt_y)
        self.allies_angles[0] = np.arctan2(dy/dx)
